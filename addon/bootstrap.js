...
function startup(data, reason) {
    // Capture install path immediately
    // Zotero/Firefox provides 'installPath' (nsIFile) in the data object
    pluginRootDir = data.installPath || data.rootDir;
    
    Zotero.debug("Better OCR: Startup (v" + data.version + ")");
    if (pluginRootDir) {
        Zotero.debug("Better OCR: Root detected at " + pluginRootDir.path);
    } else {
        Zotero.debug("Better OCR: CRITICAL ERROR - Install path not found in startup data");
    }
...
async function performOCR() {
    // Sanity check
    if (!pluginRootDir) {
        alert("Better OCR Error: Plugin did not start correctly (Root path missing). Please restart Zotero.");
        return;
    }
    ...
    if (pdfItems.length === 0) {
        alert("Better OCR: No PDF attachments found in selection.");
        return;
    }
...
async function processItem(attachmentItem) {
	...
	try {
		...
		if (await IOUtils.exists(txtPath)) {
            ...
            throw new Error("OCR tool finished but no text file was created.");
        }
	} catch (e) {
		throw e;
	}
...
function runBundledExecutable(pdfPath) {
	return new Promise((resolve, reject) => {
        // Use the global 'pluginRootDir' captured at startup
